...
namespace PortaleRegione.Client.Controllers
{
    ...
    public class NotificheController : BaseController
    {
        ...
        public async Task<ActionResult> RiepilogoNotifiche(bool is_inviate, bool archivio, int page = 1, int size = 50)
        {
            ...
            if (!is_inviate)
                model = await ApiGateway.GetNotificheRicevute(page, size, archivio);
            else
                model = await ApiGateway.GetNotificheInviate(page, size, archivio);
        ...
        {
            var destinatari = await ApiGateway.GetDestinatariNotifica(id);
