        ...
        internal static void GetBodyTemporaneo(EmendamentiDto emendamento, ref string body)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(emendamento.PATH_AllegatoTecnico))
                    allegato_tecnico = "<tr><td colspan='2' style='text-align:left;padding-left:10px'><a href='" +
                                       Path.Combine(AppSettingsConfiguration.urlPEM,
                                           emendamento.PATH_AllegatoTecnico.Replace("~/", "")) +
                                       "' target='_blank'>SCARICA ALLEGATO TECNICO</a></td></tr>";
                ...
                if (!string.IsNullOrEmpty(emendamento.PATH_AllegatoGenerico))
                    allegato_generico = "<tr><td colspan='2' style='text-align:left;padding-left:10px'><a href='" +
                                        Path.Combine(AppSettingsConfiguration.urlPEM,
                                            emendamento.PATH_AllegatoGenerico.Replace("~/", "")) +
                                        "' target='_blank'>SCARICA ALLEGATO GENERICO</a></td></tr>";
